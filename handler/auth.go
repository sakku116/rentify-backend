...
func (slf *AuthHandler) Login(ctx *gin.Context) {
	...
	if err != nil {
		slf.respWriter.HTTPJSON(ctx, 400, "bad request", err.Error(), nil)
		return
	}

	token, err := slf.authService.Login(ctx, payload.Username, payload.Password)
	if err != nil {
		slf.respWriter.HTTPError(ctx, err)
		return
	}

	slf.respWriter.HTTPJSON(ctx, 200, "OK", "", gin.H{
		"access_token": token,
	})
}
...
func (slf *AuthHandler) Register(ctx *gin.Context) {
	...
	if payload.Username == "" ||
		...
		slf.respWriter.HTTPJSON(ctx, 400, "all fields must be filled", "", nil)
		return
	}

	// match password validation
	if payload.Password != payload.ConfirmPassword {
		slf.respWriter.HTTPJSON(ctx, 400, "password and confirm password must be same", "", nil)
		return
	}
